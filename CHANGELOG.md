...
- **进行中的一轮改用 Claude Code 式的状态行**：原来宿主把「深度求索中，用时 N 秒」排在这一轮的最前面，工作过程往下越排越长、状态却停在上方。现在这一行跟在工作过程的末尾（排队中的消息仍在它下面），画成一个转动的火花标记加「用时 · 输出 tokens · 当前动作」，例如「34秒 · 1.2k tokens · 思考中…」；当前动作依次有等待模型、思考中、思考了 N 秒、输出中、准备调用工具与运行工具中。tokens 是这一轮已完成各步上报的输出用量，正在输出的那一步结束后计入。一轮结束后这一行回到宿主原来的样子与位置，折叠与展开照旧。
...
- **A Claude Code style status line for the turn in progress**: the host used to put "Deep diving for N s" at the very top of the turn, so the work grew downward while its status stayed above it. The line now follows the end of the work (queued messages still sit below it) and reads like Claude Code's: a turning spark, then elapsed time · output tokens · what the model is doing, for example "34s · 1.2k tokens · Thinking…"; the action is one of waiting for the model, thinking, thought for N s, writing, preparing a tool call and running tools. The token count is the output usage the turn's finished steps report; the step still streaming joins it once it finishes. When the turn ends the line goes back to the host's own look and place, and folding works as before.
